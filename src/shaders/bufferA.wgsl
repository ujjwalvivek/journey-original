...
fn background(uv: vec2f, t: f32) -> vec4f {
  var midlevel = 0.3;
  var disp = 0.9;
  var dist = 10.0;
...
fn fs_main(@builtin(position) position: vec4f) -> @location(0) vec4f {
  ...
  col = mix(col, vec3f(0.18, 0.12, 0.15), join);
  col = mix(col, vec3f(0.48, 0.19, 0.20), wagon);
  col = mix(col, vec3f(0.18, 0.12, 0.15), roof);
  col = mix(col, vec3f(0.38, 0.19, 0.20), loco);
  col = mix(col, vec3f(0.38, 0.19, 0.20), chem1);
  col = mix(col, vec3f(0.18, 0.12, 0.15), locoRoof);
  col = mix(col, vec3f(0.18, 0.12, 0.15), chem2 + wheel);

  var dist = 5.0;
  uv2 = uv + vec2f(t / dist + 3.5, 0.0);
  uv2.x -= t / dist * 0.2;
  let h = fbm2(uv2, 8) - 0.55;
  ...
  col = mix(vec3f(0.29, 0.09, 0.08) * smoothstep(-0.08, 0.08, uv.y), col, k);
  ...
  col = mix(col, previousCol, clamp(uniforms.controls.z, 0.0, 0.95));
